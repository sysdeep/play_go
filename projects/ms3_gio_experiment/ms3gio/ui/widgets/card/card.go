	...
	return layout.Stack{}.Layout(
		...
		layout.Expanded(func(gtx C) D {
			...
			border := widget.Border{
				Color:        widgets.MulAlpha(c.Th.Fg, 60),
						...
						return layout.Inset{
							...
							return layout.Flex{
								...
								layout.Rigid(func(gtx C) D {
									...
									return material.H5(c.Th, c.Title).Layout(gtx)
								}),
								layout.Rigid(func(gtx C) D {
									if c.Subtitle == "" {
										return D{}
								...
								layout.Rigid(func(gtx C) D {
									...
									return material.Body1(c.Th, c.Subtitle).Layout(gtx)
								}),
							)
						})
					}),
							...
							func() (flex []layout.FlexChild) {
								...
								for ii := range floatLeft {
									...
									flex = append(flex, layout.Rigid(func(gtx C) D {
										btn := material.Button(c.Th, action.Clickable, action.Label)
										btn.Color = action.Fg
